...
def export_glb(model, output_path, export_t_pose=True, animations=None, include_vertex_colors=True, textures=None, save_textures_folder=True):
    base_name = os.path.splitext(os.path.basename(output_path))[0]
    out_dir = os.path.dirname(output_path) or "."
    ...
    gltf["scenes"][0]["nodes"] = mesh_node_indices
...
def export_dae(model, output_path, export_t_pose=True, animations=None, include_vertex_colors=True):
    ...
    for m_idx, mesh in enumerate(model.meshes):
        ...
        # UVs
        uv_str = " ".join(f"{uv[0]:.4f} {1.0 - uv[1]:.4f}" for uv in mesh.uvs)
...
def export_fbx(model, output_path, export_t_pose=True, animations=None, include_vertex_colors=True):
    ...
    for m_idx, mesh in enumerate(model.meshes):
        ...
        uv_list = []
        for uv in mesh.uvs:
            uv_list.extend([f"{uv[0]:.4f}", f"{1.0-uv[1]:.4f}"])
